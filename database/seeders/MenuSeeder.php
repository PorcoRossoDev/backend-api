...
use App\Models\CustomizationMenu;
use App\Models\Menu;
...
class MenuSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $customizations = Customization::pluck('id', 'name');
        ...
        foreach ($data['menu'] as $item) {
            ...
            foreach( $item['customizations'] as $cus ){
                CustomizationMenu::create([
                    'menu_id' => $menu->id,
                    'customization_id' => $customizations[$cus]
                ]);
            }

            // attach customizations
            // $customIds = collect($item['customizations'])
            //     ->map(fn($name) => $customizations[$name]);

            // $menu->customizations()->sync($customIds);
        }
    }
}
